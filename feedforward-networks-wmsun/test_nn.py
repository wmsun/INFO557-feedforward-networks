...
def test_deep_vs_wide(capsys):
    ...
    train_in, train_out, test_in, test_out = load_hdf5("C:/Users/Wenmo/Google-Drive/INFO 557/Homework/Feedforward Networks/feedforward-networks-wmsun/data/auto-mpg.hdf5")
    ...
    assert all("mean" in loss(model).__name__ for model in [deep, wide])
    assert loss(deep) == loss(wide)
...
def test_relu_vs_tanh(capsys):
    ...
    train_in, train_out, test_in, test_out = load_hdf5("C:/Users/Wenmo/Google-Drive/INFO 557/Homework/Feedforward Networks/feedforward-networks-wmsun/data/delicious.hdf5")
    ...
    assert all(any(x in loss(model).__name__ for x in {"crossentropy", "hinge"})
               and "categorical" not in loss(model).__name__
               for model in [relu, tanh])
    assert loss(relu) == loss(tanh)
    ...
    # print("relu predict: ", relu.predict(test_in))
...
def test_dropout(capsys):
    ...
    train_in, train_out, test_in, test_out = load_hdf5("C:/Users/Wenmo/Google-Drive/INFO 557/Homework/Feedforward Networks/feedforward-networks-wmsun/data/uci-har.hdf5")
    ...
    assert all("categorical" in loss(model).__name__
               for model in [drop, no_drop])
    assert loss(drop) == loss(no_drop)
...
def test_early_stopping(capsys):
    ...
    train_in, train_out, test_in, test_out = load_hdf5("C:/Users/Wenmo/Google-Drive/INFO 557/Homework/Feedforward Networks/feedforward-networks-wmsun/data/income.hdf5")
